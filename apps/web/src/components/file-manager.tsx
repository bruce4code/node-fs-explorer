...
export function FileManager() {
  ...
  async function selectEntry(entry: FileEntry) {
    setSelected(entry); setInfo(null); setPreview(null); setHash(null); setDetailsOpen(true);
  ...
  async function loadLogs() {
    ...
    catch (reason) { setToast(reason instanceof Error ? reason.message : "日志加载失败"); }
  }
  ...
  const sidebar = <>
    ...
    <nav className="side-nav" aria-label="主导航">
      <button className="active" onClick={() => { void loadDirectory("."); setMobileNav(false); }}><FolderOpen size={17} />文件空间</button>
      <button onClick={() => { void loadLogs(); setMobileNav(false); }}><Activity size={17} />操作记录 {logs.length > 0 ? `(${logs.length})` : ""}</button>
    </nav>
